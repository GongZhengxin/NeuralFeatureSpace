...
for target_resp in [np.nan]:
    ...
    for channel in [ 62]:#range(64)28,29,60, 5, 33, 34,
        ...
        if np.abs(target_resp) > 0:
            ...
            picname = [ _ for _ in os.listdir(f"{image_savepath}/{optimal_dir}/dnntuning/") if target_flag in _ and tuningname in _][0]
            if os.path.exists(f"{image_savepath}/{optimal_dir}/dnntuning/{picname}"):
                img = Image.open(f"{image_savepath}/{optimal_dir}/dnntuning/{picname}")
                # 预处理图片
        ...
        for i in range(n_pic):
            ...
            true_savedir = f'{image_savepath}/{optimal_dir}/dnntuning/'
