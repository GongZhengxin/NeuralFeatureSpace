...
layer_roi = 'conv2-V1'
tunings = np.load(f'{work_dir}/anal/neural-selectivity/all-sub_{layer_roi}-tunings.npy',
...
for target_resp in [-1]:#[0, -2, 2, -4, 4, 6, -6, -8, 8]
    # 设置随机种子,1,2,45
    seed = 42
    allsub_mean, allsub_std = [], []
    for tuningname, tuning in tunings.items():
        
        sub = f"sub-{tuningname.split('-')[1]}"
        layer = tuningname.split('-')[2]
        roi = tuningname.split('-')[3]
        os.makedirs(f'{image_savepath}/{optimal_dir}/subtuning/{roi}', exist_ok=True)
        pics = os.listdir(f'{image_savepath}/{optimal_dir}/subtuning/{roi}') 
        if np.any([f'{tuningname}-T{target_resp}' in pic for pic in pics]):
            print(f'exists: {tuningname}-T{target_resp}')
            continue
        # 
        if 'mean' in tuningname:
            submean = np.array(allsub_mean).mean(axis=0)
            substd = np.sqrt((np.array(allsub_std)**2).mean(axis=0))
        else:
        ...
        # if tuningname != 'sub-mean-conv2-V1-roi':
        #     continue
        if 'mean' not in tuningname:
            continue
        # 读取图片
        if np.abs(target_resp) > 2:
            target_flag = f'T{target_resp - 2 * np.sign(target_resp)}'
            picname = [ _ for _ in os.listdir(f"{image_savepath}/{optimal_dir}/subtuning/{roi}") if target_flag in _ and tuningname in _][0]
            if os.path.exists(f"{image_savepath}/{optimal_dir}/subtuning/{roi}/{picname}"):
                img = Image.open(f"{image_savepath}/{optimal_dir}/subtuning/{roi}/{picname}")
                # 预处理图片
        ...
        submean_torch, substd_torch = align_datatype(input_img, submean, substd)
        #torch.randn(1, 3, 227, 227, requires_grad=True)
        optimizer = Adam([input_img], lr=4)
        # 每隔10个epoch，学习率乘以0.1
        scheduler_step = StepLR(optimizer, step_size=2000, gamma=0.9)
        # # 当指标停止改进时降低学习率
        ...
        for iteration in range(15000):
            ...
            # 如果设定了特定激活值
            if loss_act <1e-9 and target_resp is not None:
                progressing_images.append(input_img.detach().squeeze().cpu().clone())
                progressing_losses.append(-loss_act.item())
                break
        ...
        for i in range(n_pic):
            ...
            # 读取和转换图片
            images = [transform(img) for img in cropped_images]
            ...
            model(x_data)
            actmap = np.squeeze(activations[-1].detach().cpu().numpy())
            if actmap.ndim==4:
                actmap = actmap[-1]
            normed_activ = (np.sum(actmap * rfmask.cpu().numpy(), axis=(1,2))[0:63] - submean) / substd
            voxel_resp = np.dot(normed_activ, tuning) 
            voxel_response_predictions.append(voxel_resp)
            activations.clear()
            print(f'{tuningname}-T{target_resp}_{act_value}', 'resp:', voxel_resp)
            cropped_image.save(f"{image_savepath}/{optimal_dir}/subtuning/{roi}/{tuningname}-T{target_resp}_{voxel_resp}.png", 'PNG', optimize=True)
