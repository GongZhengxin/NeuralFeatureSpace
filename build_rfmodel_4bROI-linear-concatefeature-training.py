...
inputlayername = 'googlenet-concate' #'googlenet-maxpool2' # 'googlenet-inception3a' #
layer = {'name': inputlayername, 'size': net_size_info[inputlayername]}
layername = layer['name']
layername = layername.replace('.','')
layers = ['googlenet-conv2', 'googlenet-maxpool2', 'googlenet-inception3a']
...
for roi_name in ['V4']:# , ,  
    ...
    for sub in subs:
        ...
        train_features = []
        for layer in layers:
            train_feature = np.load(pjoin(concate_path, sub, f'{sub}_layer-{layer}_{roi_name}-train-feature.npy'), mmap_mode='r')
            train_features.append(train_feature)
    
        train_feature = np.concatenate(train_features, axis=2)
        train_data = np.load(pjoin(concate_path, sub, f'{sub}_layer-googlenet-conv2_{roi_name}-train-resp.npy'), mmap_mode='r')
        
        print('loading test features ')
        # test feature and response
        test_features = []
        for layer in layers:
            test_feature = np.load(pjoin(concate_path, sub, f'{sub}_layer-{layer}_{roi_name}-test-feature.npy'), mmap_mode='r')
            test_features.append(test_feature)
        test_feature = np.concatenate(test_features, axis=2)
        test_data = np.load(pjoin(concate_path, sub, f'{sub}_layer-googlenet-conv2_{roi_name}-test-resp.npy'), mmap_mode='r')
        
        if roi_name == 'V4':
            voxels = np.load(pjoin(concate_path, sub, f'{sub}_layer-googlenet-conv2_{roi_name}-voxel.npy')).tolist()
