...
layers = ['googlenet-conv2', 'googlenet-maxpool1', 'googlenet-maxpool2']
...
for inputlayername in layers:
    ...
    # axis
    axis_type = 'newretino' # 'preprocessed'
    component_path = pjoin(image_activations_path, axis_type)
    if axis_type == 'preprocessed':
        image_activations_path = pjoin(work_dir, f'prep/image_activations_{axis_type}')
    if axis_type == 'pca':
        allactivations = np.load(pjoin(component_path, f'{sub}_{inputlayername}_pca-components.npy'), mmap_mode='r')
        coco_activations = np.load(pjoin(component_path, f'{sub}_{inputlayername}_pca-components.npy'), mmap_mode='r')
    for sub in subs[1::]:
        with Timer() as t:
            ...
            if 'ca' not in axis_type:
                ...
                if axis_type == 'ica':
                    activations = np.load(pjoin(component_path, f'{sub}_{inputlayername}_ica-comps.npy'), mmap_mode='r')
                    coco_activations = np.load(pjoin(component_path, f'{sub}_{inputlayername}_ica-comps.npy'), mmap_mode='r')
                if axis_type == 'pca':
                    subidx = np.int(sub.split('-')[1])
                    activations = allactivations[4000 * (subidx-1) : 4000 * subidx]
            ...
            results = Parallel(n_jobs=27)(delayed(concate_voxel_feature)(idx, voxel) for idx, voxel in zip(idxs, voxels))
            save_indexnames = ['train-feature', 'test-feature']
            # dropout null
